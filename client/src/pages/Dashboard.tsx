...
export default function Dashboard({ user }: { user?: UserProfile }) {
  ...
  const city = (MOCK_CITIES.find(c => c.city.toLowerCase() === safeUser.city.toLowerCase()) ?? MOCK_CITIES[0]);
  ...
  return (
    <AppShell username={safeUser.username}>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="cl-card cl-card-pad hover-lift">
          <WaterDroplet stress={city.waterStress} />
        </div>
        {/* Tiles 4–6 will be added below later */}
      </div>
